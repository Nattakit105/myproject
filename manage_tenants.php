...
// 🔥 รับค่าเลขห้องที่ส่งมาจากหน้า Dashboard (ถ้ามี)
...
// 1. ส่วนของการเพิ่มผู้ใช้ใหม่ (ปรับปรุง: เพิ่มระบบเช็คซ้ำ)
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_tenant'])) {
    $room_number = $_POST['room_number'];
    $full_name = $_POST['full_name'];

    // 🚩 [เพิ่ม] ตรวจสอบก่อนว่าเลขห้องนี้มีในระบบหรือยัง
    ...
    if ($check_result->num_rows > 0) {
        // ถ้าเจอข้อมูลซ้ำ ให้แจ้งเตือนและหยุดการทำงาน
        $_SESSION['error_message'] = "❌ ไม่สามารถเพิ่มได้: เลขห้อง <strong>$room_number</strong> มีอยู่ในระบบแล้ว";
        $check_stmt->close();
    } else {
        $check_stmt->close();
        
        // ถ้าไม่ซ้ำ ถึงจะทำการสร้างบัญชีใหม่
